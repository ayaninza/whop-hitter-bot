...
def load_pool():
    if os.path.exists(ADDRESS_POOL_FILE):
        try:
            ...
            # corrupt / empty file ΓÇö discard and regenerate
            pass
...
def _resolve_state(value):
    ...
    matched nothing, and st_abbr was left as the literal "WASHINGTON" ΓÇö
        ...
        for el in locs:
            try:
                ...
                if key == "state" and tag == "select":
                    ...
                    # <input name="state"> ΓÇö a broken duplicate that could shadow the
...
def enforce_state(page, value):
    ...
    matching <select name="state">. No hidden-input injection ΓÇö that created
...
def run_checkout(checkout_url, cc, proxy=None, headless=True, tag="run"):
    ...
    email = random_email()
    name = addr["name"]   # use the SOURCE name exactly ΓÇö never invent a name
    if proxy is None:
        proxy = pick_proxy()
    print(f"[{tag}] START card ΓÇª{cc['number'][-4:]} via {proxy['server']}", flush=True)
    ...
    with sync_playwright() as p:
        ...
        # 2) fill each field ΓÇö line1 uses autocomplete to trigger browser fill
